...
from book_store.form import UserProfileForm
...
class DashBoardViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.auth_cart = Cart.objects.create(user=self.user, product=self.product, is_ordered=False)  # Authenticated user's cart

        # Create a cart for the anonymous user (session-based)
        self.anon_cart = Cart.objects.create(session_key=session.session_key, product=self.product, is_ordered=False)  # Anonymous user's cart
    ...
    def test_dashboard_get_authenticated_user(self):
        ...
        self.assertQuerysetEqual(response.context['orders'], Order.objects.filter(user=self.user, is_ordered=True), transform=lambda x: x)
        self.assertQuerysetEqual(response.context['cart'], Cart.objects.filter(user=self.user, is_ordered=False), transform=lambda x: x)
    ...
    def test_dashboard_get_anonymous_user(self):
        ...
        self.assertIsNone(response.context.get('profile_form'))  # No profile form for anonymous users
        self.assertIsNone(response.context.get('password_form'))  # No password change form for anonymous users
        self.assertQuerysetEqual(response.context['cart'], Cart.objects.filter(session_key=self.client.session.session_key, is_ordered=False), transform=lambda x: x)
        self.assertIsNone(response.context.get('orders'))  # Anonymous users don't have orders
    ...
    def test_dashboard_post_update_profile_authenticated(self):
        ...
        response = self.client.post(reverse('store:dash-board'), {
            'update_profile': 'true',
        ...
        messages = list(get_messages(response.wsgi_request))  # Use get_messages for message assertions
    ...
    def test_dashboard_post_update_address_authenticated(self):
        ...
        self.assertRedirects(response, reverse('store:dash-board'))
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), 'Your address has been updated successfully!')
